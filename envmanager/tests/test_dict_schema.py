...
from envmanager.tests.data import envloader_config_dict

config = envloader_config_dict
...
def test_mode(env):
    ...
    assert env.clear("mode") is True, "expecting true when mode popped"
    # cannot retrieve anymore or throws an error


def test_custom_validator_1(env):
    val = env("custom_validation_1")
    assert val == 'custom_validation_1', "testing custom_validation_1 failed"
    assert env.clear("custom_validation_1") is True, "expecting true when custom_validation_1 popped"
    # cannot retrieve anymore or throws an error


def test_custom_validator_2(env):
    val = env("custom_validation_2")
    assert val == 'custom_validation_2', "testing custom_validation_2 failed"
    assert env.clear("custom_validation_2") is True, "expecting true when custom_validation_2 popped"
    # cannot retrieve anymore or throws an error


def test_str_validation_abcd(env):
    val = env("str_validation_abcd")
    assert val == 'abcd', "testing str_validation_abcd failed"
    assert env.clear("str_validation_abcd") is True, "expecting true when str_validation_abcd popped"
    # cannot retrieve anymore or throws an error


def test_str_validation_1234(env):
    val = env("str_validation_1234")
    assert val == '1234', "testing str_validation_1234 failed"
    assert env.clear("str_validation_1234") is True, "expecting true when str_validation_1234 popped"
    # cannot retrieve anymore or throws an error


def test_int_validation_1(env):
    val = env("int_validation_1")
    assert val == 1, "testing int_validation_1 failed"
    assert env.clear("int_validation_1") is True, "expecting true when int_validation_1 popped"
    # cannot retrieve anymore or throws an error


def test_int_validation_minus_1(env):
    val = env("int_validation_minus_1")
    assert val == -1, "testing int_validation_minus_1 failed"
    assert env.clear("int_validation_minus_1") is True, "expecting true when int_validation_minus_1 popped"
    # cannot retrieve anymore or throws an error


def test_email_marshmallow_bytectgroup_at_gmail_dot_com(env):
    val = env("email_marshmallow_bytectgroup_at_gmail_dot_com")
    assert val == 'bytectgroup@gmail.com', "testing test_email_marshmallow_bytectgroup_at_gmail_dot_com failed"
    assert env.clear(
        "email_marshmallow_bytectgroup_at_gmail_dot_com"
...
def test_email_marshmallow_a_dot_b_at_gmail_dot_com(env):
    val = env("email_marshmallow_a_dot_b_at_gmail_dot_com")
    assert val == 'a.b@gmail.com', "testing test_email_marshmallow_a_dot_b_at_gmail_dot_com failed"
    assert env.clear(
        "email_marshmallow_a_dot_b_at_gmail_dot_com"
...
def test_set_value_3(env):
    ...
    assert env.clear('my_key_2') is True, "testing setting custom string when mode popped"
    # cannot retrieve anymore or throws an error
